...
function SidebarLeft({ titles, icons }: SidebarProps) {
  ...
  return (
    ...
    >
      <nav className="h-full flex flex-col bg-white dark:bg-gray-900 border-r dark:border-gray-700">
        ...
        <ul className="flex-1 px-2 my-3">
          {titles.map((title, index) => {
            ...
            return (
              <li
                key={title}
                className={`relative items-center py-2 px-3 my-1 font-medium rounded-md cursor-pointer transition-colors dark:text-whites group ${
                  selectedIndex === index
                ...
                <Link href={"#"} className="flex items-center">
                  ...
                  {isLast ? (
                    subOpened ? (
                      ...
                      <button
                        className="hover:bg-blue-500 rounded-lg dark:text-white"
                        onClick={() => {
                          setSubmenuOpen((open) => !open);
                        }}
                      >
